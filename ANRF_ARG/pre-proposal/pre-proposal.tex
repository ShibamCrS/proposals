...
Constraint-solving tools --- CP, SAT, and MILP --- have proven powerful for uncovering new distinguishers and key-recovery attacks in conventional ciphers, but their systematic application to the low-latency cipher family is nascent.
...
Hardware-based mitigations for microarchitectural attacks --- memory encryption, pointer authentication, secure caches --- increasingly rely on low-latency primitives.
...
Deploying comprehensive protections --- duplication or physical shielding --- across the entire unrolled circuit is complex and costly.
...
Prior publications on fault analysis of GIFT-family ciphers~\owncite{Kundu_Ghosh_Aikata_Saha_2025} have involved end-to-end experimental validation, from distinguisher derivation through to actual key recovery on hardware.
...
\begin{enumerate}[label=\textbf{D\arabic*.}, leftmargin=*, itemsep=2pt]
  \item \textbf{Cryptanalytic results on deployed ciphers.}
        Published attacks --- new distinguishers and key-recovery results --- on at least two of the target ciphers (\tx{ARADI}, \tx{ChiLow}, \tx{SCARF}, \tx{QARMAv2}), targeting publication at CRYPTO, EUROCRYPT, ASIACRYPT, or IACR ToSC.
